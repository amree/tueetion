...
namespace :resque do
  ...
  def run_worker(queue, count = 1)
    ...
    ops = {:pgroup => true, :err => [(Rails.root + "log/resque.err.log").to_s, "a"],
                            :out => [(Rails.root + "log/resque.stdout.log").to_s, "a"]}
    env_vars = {"QUEUE" => queue.to_s, 'RAILS_ENV' => Rails.env.to_s, 'TERM_CHILD' => 1.to_s}
